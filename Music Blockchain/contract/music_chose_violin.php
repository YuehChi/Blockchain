<?php
    ...
    if(isset($_POST["p1"])&&($_POST["p1"]=="確認資訊")){
        ...
        if( $count < 0 ){
            echo "<script>alert('警告：將在確認之後跳頁'); location.href = 'music_chose_violin.php';</script>";
        }elseif( $type == 2 AND  $count < 7){
            echo "<script>alert('警告：您選擇的是週方案，請確認日數大於七天'); location.href = 'music_chose_violin.php';</script>";
        }elseif( $type == 3 AND $count < 30){
            echo "<script>alert('警告：您選擇的是月方案，請確認日數大於三十天'); location.href = 'music_chose_violin.php';</script>";
        } else{
            $_SESSION['count'] = $count;
            $_SESSION['startDate'] = $startDate;
            $_SESSION['endDate'] = $endDate;
            $_SESSION['type'] = $type;
            header("Location: ./music_tranfer_violin.php");
        }
     }
